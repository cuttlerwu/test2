...
# 超参数设置
EPOCH = 200   #遍历数据集次数
pre_epoch = 135  # 定义已经遍历数据集的次数
BATCH_SIZE = 128	  #批处理尺寸(batch_size)
LR = 0.01		#学习率
new_lr = [0.001,0.0005,0.0001]   
...
#载入已保存的模型参数
net.load_state_dict(torch.load('./model/net_135_VGG.pth'))
	...
	with open("acc.txt", "w") as f:
		with open("log.txt", "w")as f2:
			for epoch in range(pre_epoch, EPOCH):
				...
				# 修改 lr
				#################################################
				if epoch == 160: nlr = new_lr[1]
				if epoch == 180:
					nlr = new_lr[2]
					trainloader = torch.utils.data.DataLoader(trainset, batch_size=64, shuffle=True, num_workers=0)
				if epoch == 160 or epoch == 180: 
